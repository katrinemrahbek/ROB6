			...
			while (n_frames < 51)
			{
				...
				if(hasArduinodata)
				{
					...
					if(listener->hasData)
					{
						...
						if(n_frames == 1)
						{
							sumCloud = DepthDataToPCL(&(listener->data));
						}
						else
						{
							pcl::PointCloud<pcl::PointXYZ> sum;
							pcl::IterativeClosestPoint<pcl::PointXYZ, pcl::PointXYZ> icp;
							icp.setInputSource(DepthDataToPCL(&listener->data));
							icp.setInputTarget(sumCloud);
							icp.setMaximumIterations(50);
							std::cout << "aligning clouds, sum size: " << sumCloud->points.size() << ", " << icp.getInputSource()->points.size() << "\n";
							icp.align(sum);
							for(auto p : sum.points)
								sumCloud->points.push_back(p);
						}
						listener->writePLY(folderName + "/sum_cloud_" + std::to_string(n_frames) + ".ply", sumCloud);
						*/
						picoMut.unlock();
